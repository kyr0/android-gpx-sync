...
public class RemoteApiAdapter {
    ...
    private static final String BASE_URL = "http://www.aron-homberg.de";
    private static final String TAG = "RemoteApiAdapter";
    private static final String API_KEY = "531F7B6E-0DB0-49F6-99A9-95DB7132B123";
    private static final String UPLOAD_URL = BASE_URL + "/journey/track/upload";
    private static final String POS_SEND_URL = BASE_URL + "/journey/position/update";
    private static final String LOG_SYNC_URL = BASE_URL + "/journey/log/create";
    private static final String GET_LOG_IDS_URL = BASE_URL + "/journey/log/list";

    private static boolean precheck(final MainActivity ctx) {

        if (!isOnline(ctx)) {
            ctx.runOnUiThread(() -> ctx.toast("Sorry, I'm offline :-("));
            return false;
        }
        return true;
    }
    ...
    public static void callSyncTracksApi(final ArrayList<GpxTrack> trackList, final MainActivity ctx) {
        ...
        new Thread(new Runnable() {
            ...
            private boolean isAlreadySynced(JSONArray logIds, String logId) {
                    ...
                    public void onSuccess(int statusCode, cz.msebera.android.httpclient.Header[] headers, byte[] response) {
                        ...
                        try {
                            ...
                            ctx.runOnUiThread(() -> {
                                ...
                                Button syncButton = ctx.findViewById(R.id.syncButton);
                                ...
                                for (final LogEntry l : logEntries) {
                                    ...
                                    client.post(LOG_SYNC_URL, logParams, new AsyncHttpResponseHandler() {
                                        ...
                                        public void onFailure(int statusCode, cz.msebera.android.httpclient.Header[] headers, byte[] responseBody, Throwable error) {
                                            ...
                                            ctx.runOnUiThread(() -> ctx.toast("Log sync FAILED: " + l.getMessage() + "; HTTP status: " + statusCode));
                                        }
    ...
    private static boolean isOnline(Context ctx) {
        ...
        return Objects.requireNonNull(conMgr).getNetworkInfo(0).getState() == NetworkInfo.State.CONNECTED
